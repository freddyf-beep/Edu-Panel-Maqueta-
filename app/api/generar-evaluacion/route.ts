...
async function generateText(provider: AIProvider, body: EvalCopilotRequest, prompt: string, signal?: AbortSignal): Promise<string> {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rawText = await generateText(provider, body, prompt, req.signal)
    ...
    try {
      ...
      try {
        const retryText = await generateText(provider, body, retryPrompt, req.signal)
