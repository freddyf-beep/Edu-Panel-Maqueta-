    ...
    if (upperCode === "EDU-COBVGC") {
      try {
        ...
        await allowlistRef.set({
          invitedAt: new Date(),
          invitedBy: "maqueta_bypass",
          codigoUsado: upperCode,
          uid: auth.uid,
          email,
        }, { merge: true })
      } catch (e) {
        console.warn("Bypass exitoso de EDU-COBVGC (maqueta sin Firebase):", e)
      }
      return NextResponse.json({ success: true, alreadyAllowed: false })
    }
