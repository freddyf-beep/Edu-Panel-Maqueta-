...
function addFilesToZip(dirPath, rootPath = '') {
  const items = fs.readdirSync(dirPath);
  for (const item of items) {
    const fullPath = path.join(dirPath, item);
    const relPath = path.join(rootPath, item).replace(/\\/g, '/'); // Normalizar barras para el zip
...
console.log("Iniciando compresión de la maqueta EduPanel...");
const srcDir = path.resolve('.');
addFilesToZip(srcDir);

console.log("Generando archivo ZIP comprimido...");
zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE', compressionOptions: { level: 6 } })
  .then((content) => {
    const dest = path.join(srcDir, 'edupanel_maqueta.zip');
